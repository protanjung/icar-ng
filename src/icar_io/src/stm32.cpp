...
class STM32 : public rclcpp::Node {
   private:
    //-----Parameter
    std::string stm32_ip;
    int stm32_port;
   ...
   public:
    STM32() : Node("stm32") {
        //-----Parameter
        this->declare_parameter<std::string>("stm32_ip", "192.168.50.2");
        this->declare_parameter<int>("stm32_port", 9798);
        this->get_parameter("stm32_ip", stm32_ip);
        this->get_parameter("stm32_port", stm32_port);
        //-----Timer
        tim_50hz = this->create_wall_timer(20ms, std::bind(&STM32::cllbck_tim_50hz, this));
        //-----Subscriber
        sub_stm32frompc_throttle_steering = this->create_subscription<std_msgs::msg::Int16MultiArray>(
            "stm32frompc/throttle_steering", 10,
            std::bind(&STM32::cllbck_sub_stm32frompc_throttle_steering, this, std::placeholders::_1));
        sub_stm32frompc_transmission = this->create_subscription<std_msgs::msg::Int8>(
            "stm32frompc/transmission", 10,
            std::bind(&STM32::cllbck_sub_stm32frompc_transmission, this, std::placeholders::_1));
        sub_stm32frompc_accessory = this->create_subscription<std_msgs::msg::UInt8>(
            "stm32frompc/accessory", 10,
            std::bind(&STM32::cllbck_sub_stm32frompc_accessory, this, std::placeholders::_1));
    ...
    bool stm32_init() {
        ...
        /* The code is creating a socket for communication with the STM32 device. */
        if ((socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
            RCLCPP_ERROR(this->get_logger(), "Socket creation error");
            return false;
        }

        /* The code is setting up the server address for the socket communication. */
    ...
    bool stm32_routine() {
        ...
        int len_data_from_pc = sendto(socket_fd, socket_tx_buffer, 12, MSG_DONTWAIT,
                                      (const struct sockaddr*)&socket_server_address, socket_tx_len);

        /* The code is receiving data from the STM32 device. */
        int len_data_to_pc = recvfrom(socket_fd, socket_rx_buffer, 1024, MSG_DONTWAIT,
                                      (struct sockaddr*)&socket_client_address, (socklen_t*)&socket_rx_len);
        ...
        if (len_data_from_pc < 0) {
            // RCLCPP_ERROR(this->get_logger(), "Failed to send data to STM32");
            return true;
        }

        if (len_data_to_pc < 0) {
            // RCLCPP_ERROR(this->get_logger(), "Failed to receive data from STM32");
            return true;
        }
        ...
        if (epoch_to_pc != last_epoch_to_pc + 1) {
            RCLCPP_ERROR(this->get_logger(), "Epoch is not sequential");
            return true;
        }
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<STM32>();
    rclcpp::spin(node);
    rclcpp::shutdown();
